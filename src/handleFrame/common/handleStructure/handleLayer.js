    ...
    if (item.children && item.children.length == 1 && !(item.type == 'Mask' && item.style['border-radius'])) {
        ...
        if (item.children[0].type == Container || (!item.children[0].type && (item.children[0].sign === '__li' || item.children[0].sign === '__oth'))) {
            if (!item.children[0].textContainer &&
                ...
                !item.children[0].style['background-color'] &&
                !item.children[0].style['background-position'] &&
                !item.children[0].style['background-size'] &&
            ...
            //Container 子元素有背景或者边框 且父元素不为artboard
                ...
                //父元素无背景及边框-删除父元素
                if (item.type == Container && !item.style['border'] && !item.style['background'] && !item.style['background-color'] && !item.style['background-image'] && !item.style['background-position'] && !item.style['background-size'] && !item.style['background-repeat'] && !item.style['border-radius']) {
                    item = item.children[0];
                    item = handleSingleLayer(item);
                    //子元素有背景色，父子结构大小完全相同-删除父元素
                /* eslint-disable */
                } else if (item.children[0].style['background-color'] &&
                    /\,1\)$/.test(item.children[0].style['background-color']) && 
        ...
        //Image
            ...
            if (item.type == Container) {
                if (!item.style['border'] && !item.style['background'] && !item.style['background-color'] && !item.style['background-image'] && !item.style['background-position'] && !item.style['background-size']) {
                    item = item.children[0];
                    item = handleSingleLayer(item);
                    //子元素与父元素完全相同-删除父元素
                } else if (item.children[0].height == item.height &&
                    item.children[0].width == item.width &&
                    item.children[0].x == item.x &&
                    item.children[0].y == item.y &&
