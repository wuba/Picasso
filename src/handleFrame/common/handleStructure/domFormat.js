function domFormat(data) {
    ...
    function sortData(data) {
        ...
        for (let i = 0; i < len; i++) {
            for (let j = 0; j < len - 1; j++) {
                const { x: x1, y: y1, width: width1, height: height1 } = data[j];
                const { x: x2, y: y2, width: width2, height: height2 } = data[j + 1];
                if (x1 === x2 && y1 === y2) {
    ...
    function getMax(arr, edgeFlag = true) {
        // 分组
        ...
        while (len) {
            ...
            const maxArr = arr.filter(({ width, height }) => {
                // 大小容差
    ...
    function groupData(arr, p) {
        ...
        if (groupLayers[1] && Array.isArray(groupLayers[1]) && groupLayers[1].sign === '__list') {
        ...
        if (maxN === 1) {
            ...
            const segArr = [];
            arr.forEach((d, i) => {
                if ((d.width === mwidth && d.height < 5) ||
                    (d.height === mheight && d.width < 5)) segArr.push(i);
            });
        ...
        if (p) {
            ...
            p.children.forEach((pc) => {
                ...
                if (pc.children && pc.children.length > 0 && pc.children.sign !== '__li') {
