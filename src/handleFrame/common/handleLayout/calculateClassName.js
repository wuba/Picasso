...
const isVerticalList = (data) => {
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        if (Math.abs(currItem.width - firstItem.width) > 3) {
            return false;
        }
        if (Math.abs((currItem.x + currItem.width * 0.5) - (firstItem.x + firstItem.width * 0.5)) > 3) {
            return false;
        }
    }
    return true;
 ...
 * @param {Object} parent 
 */
const calculateVerticalList = (data) => {
    const firstItem = data[0];
    for (let i = 1; i < data.length; i++) {
        const currItem = data[i];
        if (Math.abs(currItem.height - firstItem.height) > 2) {
            data[i].height = firstItem.height;
        }
    }
    data = data.map(item => {
        item.class_name = 'li';
 ...
 * @param {*} parent 
 */
const calculateLeftImgRightInfo = (data) => {
 ...
 * @param {*} parent 
 */
const handleContinuousListItem = (data) => {
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        //是否相同
        if (
            Math.abs(currItem.width - currStartItem.width) < 3 &&
            Math.abs(currItem.height - currStartItem.height) < 3 &&
            Math.abs((currItem.x + currItem.width * 0.5) - (currStartItem.x + currStartItem.width * 0.5)) < 3 &&
