const twoPointsAngle = require('./twoPointsAngle');
...
module.exports = (gradient, layer, fillStyle, contextSettings = 1) => {
    ...
    let endColor = {};
    //计算向量距离
    function verticalDistance(point1, point2, lineAngle) {
    ...
    let valList = changePointList.map(({
        color,
        position
    }) => {
        let {
    ...
    gAngle = gAngle / Math.PI * 180;
    style['background'] = `linear-gradient(${Math.round(gAngle * 10) / 10}deg,${valList.join(',')})`;
    return style;
