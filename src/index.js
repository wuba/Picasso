    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            global.timer = setInterval(async () => {
                ...
                if (appVersion == sketchVersion && (!foreignSymbols || !Array.isArray(foreignSymbols) || (Array.isArray(foreignSymbols) && foreignSymbols.length == 0))) {
                    ...
                    if (fs.existsSync(`${cliSketchParseResultPath}/${sketchName}`)) {
                        fileSystem.rmdirSync(`${cliSketchParseResultPath}/${sketchName}`);
                    }
                    // 有画板名称的画板(有效画板)
