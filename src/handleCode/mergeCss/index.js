...
const handleItemChild = (list) => {
    ...
    if (Array.isArray(firstItem.children)) {
        ...
        firstItem.children.forEach((item, index) => {
            ...
            if (Array.isArray(secondItem.children) && secondItem.children.length > 0) {
                try {
                    let flag2 = true;
                    list.forEach(item => {
                        if (item.children[secondIndex].children === undefined) {
                            flag2 = false;
                        } else if (item.children[secondIndex].children.length !== secondItem.children.length) {
                            flag2 = false;
                        }
                    });
                    if (flag2) {
                        secondItem.children.forEach((item2, index2) => {
                            const itemArr = [];
                            list.forEach(ite => {
                                itemArr.push(ite.children[secondIndex].children[index2]);
                            });
                            const commonStyle = mergeCommonStyle(itemArr);
                            const classObj2 = {};
                            list.forEach((ite, ind) => {
                                ite.children[secondIndex].children[index2].commonStyle = commonStyle;
                                const currOnlyStyle = getOnlyStyle(ite.children[secondIndex].children[index2].style, commonStyle);
                                ite.children[secondIndex].children[index2].onlyStyle = currOnlyStyle;
                                ite.children[secondIndex].children[index2].className = secondItem.children[index2].className;
                                if (Object.keys(currOnlyStyle).length > 0) {
                                    if (classObj2[JSON.stringify(currOnlyStyle)] === undefined) {
                                        //独有className
                                        ite.children[secondIndex].children[index2].subClassName = `subli${ind}`;
                                        classObj2[JSON.stringify(currOnlyStyle)] = `subli${ind}`;
                                    } else {
                                        ite.children[secondIndex].children[index2].subClassName = classObj2[JSON.stringify(currOnlyStyle)];
                                        ite.children[secondIndex].children[index2].onlyStyle = {};
                                    }
                                }
                            });
