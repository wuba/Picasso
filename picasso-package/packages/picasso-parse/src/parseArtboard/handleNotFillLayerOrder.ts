...
const handleNotFillLayerOrder = (layers:SKLayer[]): SKLayer[] => {
    ...
    layers.sort((bLayer: SKLayer, aLayer: SKLayer) => {
        const aLayerIsTouMing = aLayer.style?.fills?.filter(item=>item.isEnabled).length === 0;
        const bLayerIsTouMing = bLayer.style?.fills?.filter(item=>item.isEnabled).length === 0;
