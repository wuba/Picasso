...
| [`restore-dsl-rendering-guide.md`](restore-dsl-rendering-guide.md) | **消费端渲染语义规范**——拿到合法 DSL 后怎么渲染才对。适用于所有 RestoreDSL 消费方：服务端 LLM 还原提示词、确定性渲染器（`tools/gen_restore_html.js` / `tools/gen_restore_fragment.js`）、diff 可视化等 |
...
- 与 `src/parseRestoreDSL/restoreTypes.ts` 的 `RESTORE_SCHEMA_VERSION`、`../tools/gen_restore_html.js`、`../tools/gen_restore_fragment.js` 同步更新——改任一处都要检查其余几处。
